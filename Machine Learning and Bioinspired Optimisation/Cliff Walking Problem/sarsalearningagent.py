...
class SARSA():
    ...
    def update_qvalue(self, x, y, action, q_prime, reward):
    ...
    def get_action(self, x, y, epsilon_decay, episode):
        if epsilon_decay == True:
            epsilon = self.epsilon / np.sqrt(episode + 1)
        else:
    ...
    def train(self, episodes, display_world, epsilon_decay):
        ...
        for episode in range(episodes):
            ...
            while reached_goal == False:
                ...
                a_prime = self.get_action(self.agent.x, self.agent.y, epsilon_decay, episode)
                q_prime = self.get_qvalue(self.agent.x, self.agent.y, a_prime)
                
                # Update the Q Values
                self.update_qvalue(cur_x, cur_y, action, q_prime, reward)
                ...
                # Set next action equal to a_prime
                action = a_prime
