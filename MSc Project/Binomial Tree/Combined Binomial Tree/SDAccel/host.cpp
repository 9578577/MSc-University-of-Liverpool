...
int main(int argc, char **argv) {
    ...
    // Allocate memory
    std::vector<DATA_TYPE, aligned_allocator<DATA_TYPE>> output(2 * MAX_ENTRIES * sizeof(DATA_TYPE));
    ...
    for(int i = 0; i < num_options; i++) {
        ...
        std::cout << "[" << i << "] S " << S[i] << "; K " << K[i] << "; T " << T[i] << "; D " << D[i] << "; r "
                  << r[i] << "; v " << v[i] << "; type " << type[i] << "; height " << height[i] << std::endl;
    }
    ...
    OCL_CHECK(err,
              cl::Buffer buffer_output(context,
                                    ...
                                    2 * MAX_ENTRIES * sizeof(DATA_TYPE),
									output.data(),
                                    &err));
    OCL_CHECK(err,
    ...
    for(int i = 0; i < 2 * num_options; i++) {
        std::cout << std::setprecision(8) << "[" << i << "] Option price: " << output[i] << std::endl;
    }

    std::cout << std::setprecision(8) << "Time taken " << (float)(kernel_end - kernel_start) /  1000000000 << " seconds" << std::endl;
