 ...
 * General Addititve Binomial Valuation of European and US Options
 */
...
DATA_TYPE us_binomial_tree(DATA_TYPE S,
						   DATA_TYPE K,
						   DATA_TYPE T,
						   DATA_TYPE D,
						   DATA_TYPE r,
...
#pragma HLS inline off
    ...
    // Compute starting constants
    DATA_TYPE dt = (DATA_TYPE)T / height;
    DATA_TYPE u = 1 + ((r - D) * dt) + (v * sqrt(dt));
    DATA_TYPE d = 1 + ((r - D) * dt) - (v * sqrt(dt));
    DATA_TYPE disc = 1 + (-r * dt);
    DATA_TYPE pu = (1 + ((r - D) * dt) - d) / (u - d);
    ...
    power_calculation: for(int i = 1; i < height; i++) {
	#pragma HLS loop_tripcount min=1000 max=30000
	#pragma HLS unroll
	#pragma HLS pipeline
	#pragma HLS dependence variable=temp1 inter RAW distance=1 true
	#pragma HLS dependence variable=temp2 inter RAW distance=1 true
    ...
    initialise_option_value: for(int i = 0; i < height; i++) {
	#pragma HLS loop_tripcount min=3000 max=3000 avg=3000
	#pragma HLS unroll factor=2
	#pragma HLS pipeline

        St[i] = S * temp1[height - 1 - i] * temp2[i];
    }

    // Initialise option values at maturity
    if(type == 0) {
        call_price_maturity: for(int i = 0; i < height; i++) {
        #pragma HLS loop_tripcount min=3000 max=3000 avg=3000
        #pragma HLS unroll factor=2
        #pragma HLS pipeline
        ...
        put_price_maturity: for(int i = 0; i < height; i++) {
        #pragma HLS loop_tripcount min=3000 max=3000 avg=3000
        #pragma HLS unroll factor=2
        #pragma HLS pipeline
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    DATA_TYPE temp_output[2 * MAX_ENTRIES];
...
#pragma HLS ARRAY_PARTITION variable=temp_height cyclic factor=2 dim=1
    ...
    // Calculate eu_binomial_tree() and us_binomial_tree() in parallel
	#pragma HLS dataflow
	...
	#pragma HLS unroll factor=2
    	...
    	temp_output[i + n_options] = us_binomial_tree(temp_S[i],
													    temp_K[i],
													    temp_T[i],
													    temp_D[i],
													    temp_r[i],
    ...
    copy_host: for(int i = 0; i < 2 * n_options; i++) {
	#pragma HLS loop_tripcount min=1 max=25
	#pragma HLS pipeline
