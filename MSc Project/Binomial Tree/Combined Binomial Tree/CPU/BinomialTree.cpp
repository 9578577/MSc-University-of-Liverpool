 ...
 * General Addititve Binomial Valuation of American and European Options
 */

#include "BinomialTree.h"

DATA_TYPE eu_binomial_tree(DATA_TYPE S, DATA_TYPE K, DATA_TYPE T, DATA_TYPE D, DATA_TYPE r, DATA_TYPE v, int type, int height) {
...
DATA_TYPE us_binomial_tree(DATA_TYPE S, DATA_TYPE K, DATA_TYPE T, DATA_TYPE D, DATA_TYPE r, DATA_TYPE v, int type, int height) {
...
int main() {
    ...
    DATA_TYPE output[2 * MAX_ENTRIES];
    ...
    for(int j = 0; j < i; j++) {
        output[j] = eu_binomial_tree(S[j], K[j], T[j], D[j], r[j], v[j], type[j], height[j]);
        output[j + i] = us_binomial_tree(S[j], K[j], T[j], D[j], r[j], v[j], type[j], height[j]);
    }
